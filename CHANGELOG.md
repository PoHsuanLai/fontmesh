...
- `FontMeshError::GlyphNotFound` and `FontMeshError::LinearizationFailed`. Neither variant was ever returned by the public API (`glyph_id` uses `Option` for missing characters; `linearize_outline` always succeeds).
